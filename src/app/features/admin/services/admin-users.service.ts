 ...
 * `streamer.API#59`/`#61`) — `getUsers()` (`GET /admin/users`, пагинация +
 * опциональные фильтры `login`/`role`, `#61`), `getUser(id)` (`GET
 * /admin/users/:id`, полная карточка — профиль + игровые аккаунты + соц-сети,
 * `#61`), `updateRole()` (`PATCH /admin/users/:id/role`, только `USER`/`ADMIN`
 * — `MODERATOR` backend отклоняет `400`), `remove()` (`DELETE
 * /admin/users/:id`). Все защищены `JwtAuthGuard`+`RolesGuard(ADMIN)` на
 * backend; единственный потребитель — `AdminUsersPage`.
 */
...
export class AdminUsersService {
  ...
  getUsers(
    ...
    return this.api.get<PaginatedResponse<AdminUser>>('/admin/users', {
      page,
      limit,
      ...(filter?.login && { login: filter.login }),
