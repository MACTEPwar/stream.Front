...
/**
 * Сайдбар-меню (`stream.Front#74`) сгруппировано по разделам — пока
 * единственная группа «Справочники» (`schedule` → `stream.Front#76`,
 * `users` → `stream.Front#77`), сворачиваемая (по прямому запросу
 * пользователя при расширении #77) — группировка задела на будущее,
 * когда появятся не-справочник разделы админки.
 */
